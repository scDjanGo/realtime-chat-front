...
import { useCurrentChat, useMyUserStore, useOpenMainBurger } from "@/store";
...
export default function Current_Chat_Room({
  ...
  const sendMessage = () => {
    if (!input.trim() || !wsCurrent) return;
    wsCurrent.send(
      JSON.stringify({ type: "private", to: String(uuid), message: input })
    );
    setInput("");
  };
